...
local getSteps = function(indices, refSizes, size)
	if #indices == 1 then
		return { { idx = 1, refSize = refSizes[1] } }
	end
	...
	local numRepeat = math.floor(repeatSize / refSizes[indices[2]] + .5)
	...
	local steps = { }
	if indices[1] > 0 then table.insert(steps, { idx = indices[1], refSize = scale * refSizes[indices[1]] }) end
	for j = 1, numRepeat do table.insert(steps, { idx = indices[2], refSize = scale * refSizes[indices[2]] }) end
	if indices[3] > 0 then table.insert(steps, { idx = indices[3], refSize = scale * refSizes[indices[3]] }) end
...
function bridgeutil.getRailingInterval(params, pillarIndex, before)
	...
	for i = 1, #params.railingIntervals do
		...
		if before and interval.hasPillar[1] then count = count + 1 end
		if not before and interval.hasPillar[2] then count = count + 1 end
		
		if count == pillarIndex then return interval end
	end
...
function bridgeutil.configurePillar(modelData, pillarModels, height, width)
	...
	for i = 1, #pillarModels do 
		...
		assert(#pillarModels[i] == 1 or #pillarModels[i] == 2)
		result.indices2[i] = #pillarModels[i] == 2 and { 1, 2, 1 } or { 1 }
		result.flipped[i] = #pillarModels[i] == 2 and { false, false, true } or { false }
	end
...
function bridgeutil.configureRailing(modelData, interval, railingModels, length, width)
	...
	local hasCollModels = #railingModels[1] == 5 -- TODO HACK 3 side and 2 rep
	...
	result.indices1 = { interval.hasPillar[1] and 1 or -1, 2, interval.hasPillar[2] and 3 or -1 }
	result.indices2 = { }
	...
	for i = 1, #railingModels do 
		...
		result.indices2[i] = { rightIdx, repIdx, leftIdx }
		result.flipped[i] = { false, false, true }
	end
...
function bridgeutil.repeat2D(modelData, config)
	...
	local steps1 = getSteps(config.indices1, config.refSizes1, config.size1)
	...
	for i = 1, #steps1 do
		...
		local steps2 = getSteps(config.indices2[idx1], config.refSizes2[idx1], config.size2)
		...
		for j = 1, #steps2 do
			...
			local flipped = config.flipped[idx1][idx2]
			
			local flipped = #steps2 > 1 and j == #steps2 -- TODO HACK
			...
			local offset2 = flipped and size2 or .0
...
function bridgeutil.makeDefaultUpdateFn(data)
	return function(params)
		...
		for i = 1, #params.railingIntervals do
			local interval = params.railingIntervals[i]
			local railingConfig = bridgeutil.configureRailing(modelData, interval, railingModels, interval.length, params.railingWidth)
