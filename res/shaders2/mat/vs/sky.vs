...
void main() {
	...
	pos.xyz *= skySize;
	pos.xyz += vec3(u_view.viewInverse[3]);

	gl_Position = u_view.projView * pos;
#ifdef RESCALE_Z_POS
	gl_Position.z = (.5 * gl_Position.z + .5 * gl_Position.w);
#endif
	gl_Position.z = 1.0; // Prevent clipping
#ifdef INVERT_Y
	gl_Position.y = -gl_Position.y;
#endif

	texCoord = vec3(u_light.lightInvTransf * vec4(attrPosition.xyz, 1.0));
}
