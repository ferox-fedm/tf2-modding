...
VERTEX_INPUT(ATTR_NRML_LOC, vec3, attrNormal, vec3(0.0));
VERTEX_INPUT(ATTR_TNGT_LOC, vec4, attrTangent, vec4(0.0));
VERTEX_INPUT(ATTR_INFLUENCE_LOC, vec4, attrInfluence, vec4(0.0));

const vec3 defaultInstAttrCblendColors[] = vec3[](vec3(0.0), vec3(0.0), vec3(0.0), vec3(0.0));
VERTEX_INPUT(INSTATTR_CBLEND_LOC, vec3, instAttrCblendColors[4], defaultInstAttrCblendColors);
...
OUTPUT(CBLEND_LOC, vec3, cblendColors[4]);
...
#endif
	normal_ = normalize(vec3(/*instAttrModel **/ skinnedNrml));
	tangent_ = normalize(vec3(/*instAttrModel **/ skinnedTangent));
