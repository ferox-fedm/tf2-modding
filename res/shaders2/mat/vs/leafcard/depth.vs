...
void main() {
	...
	float distToCam = calcDistToBox(u_view.camPos, instAttrBbox);
	float lodParam = clamp((distToCam - smoothLodRange.x) / (smoothLodRange.y - smoothLodRange.x), 0, 1);

	float fadeOutAlpha = compFadeOutAlpha(distToCam, u_vmat.fadeOutRanges[0] * vec2(u_view.lodFactor, 1.0 / u_view.lodFactor));
	vec3 fadeScale = vec3(fadeOutAlpha, fadeOutAlpha, 1.0);

	vec3 pos = mix(attrOffset, attrSmoothLodOffet, lodParam);
	
	vec4 posEye = u_view.view * centerWorld + instAttrModel * vec4(fadeScale * pos, .0);
