...
LAYOUT_SAMPLER(1, 29) uniform sampler2D foamNormalTex;
...
void main() {
	vec3 foamNormal = decodeNormalScale(texture(foamNormalTex, texCoordAlpha.xy).rg, 3.0);

	float alpha = posAmbient.w;
	
	fragmentOutputCustomNormal(vec4(foamNormal, alpha));
}
