...
LAYOUT_SAMPLER(0, 22) uniform samplerCube skyTex;
...
INPUT(TC_LOC, vec3, texCoord, vec3(1.0));

void fragmentOutputCustom(vec4 color, vec4 normal);

void main() {
	gl_FragDepth = 1.0;
	fragmentOutputCustom(u_light.lightScale * texture(skyTex, texCoord), vec4(u_view.viewInverse[2].xyz, 1.0));
}
