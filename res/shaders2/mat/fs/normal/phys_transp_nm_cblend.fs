...
void main() {
	...
	vec3 albedo = colorBlend(col.rgb, cblendValue, u_mat.cblendScale, cblendColor);

	vec3 metalGlossAo = texture(metalGlossAoTex, texCoordAlpha.xy).rgb;

	float ambient = min(posAmbient.w, metalGlossAo.b);

	fragmentOutput(posAmbient.xyz, ambient, normal, albedo * u_mat.albedoScales[matIndex].xyz, metalGlossAo.r, metalGlossAo.g, alpha);
}
