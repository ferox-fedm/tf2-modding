...
void main() {
	vec4 matCoeff = u_noTex.matCoeff;
	vec4 color = u_noTex.color;

	vec3 normal = normalize(normal_);

	vec4 albedoGloss = getAlbedoAndGlossForLegacyMatCoeff(matCoeff, color.rgb * vertexColor.rgb);

	fragmentOutput(posAmbient.xyz, posAmbient.w, normal, albedoGloss.rgb, .0, albedoGloss.a, color.a * vertexColor.a);
}
