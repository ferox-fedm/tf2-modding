...
LOCATION(0) in VertexDataOut {
	vec4 positionAndHeightMapOffset;
}
inData[];
...
void main() {
	...
	uv.y = 1 - uv.y;
	vec2 patchPos = inData[0].positionAndHeightMapOffset.xy;

	float x = patchPos.x + getPatchSize() * uv.x;
	float y = patchPos.y + getPatchSize() * uv.y;
	float z = getHeightAt(inData[0].positionAndHeightMapOffset.zw, uv);

	gl_Position = u_view.projView * vec4(x, y, z, 1.0);
	depth = gl_Position.z;
