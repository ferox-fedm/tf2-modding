...
LAYOUT_SAMPLER(1, 26) uniform sampler2DArray colorArrayTex;
// uniform sampler2DArray translucencyTex; // currently not used
// uniform sampler2DArray normalTex;		// currently not used
LAYOUT_SAMPLER(1, 28) uniform sampler2DArray glossTex;
LAYOUT_SAMPLER(1, 29) uniform sampler2DArray overlayTex;
...
void main() {
	...
	vec4 col = texture(colorArrayTex, texCoordGrassType);
	if (col.a < u_push.alphaThreshold) discard;
		
	vec4 scaleStrength = u_matDataBlock.mat2grassOverlayScaling[grassMatType.y];
	vec3 col1 = texture(overlayTex, vec3(vpos.xy * scaleStrength.y, grassMatType.y)).rgb; // use matIndex
	col.rgb = overlay(vec4(col.rgb, scaleStrength.z), col1) * .7;
