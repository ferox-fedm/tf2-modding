...
layout(location = 0) in vec4 positionAndHeightMapOffset[];
layout(location = 1) in vec4 tessLevelOuter[];
layout(location = 2) in vec2 tessLevelInner[];
...
void main() {
	int i;
	for (i = 0; i < 4; ++i) {
		gl_TessLevelOuter[i] = tessLevelOuter[gl_InvocationID][i];
	}
	
	for (i = 0; i < 2; ++i) {
		gl_TessLevelInner[i] = tessLevelInner[gl_InvocationID][i];
	}
	
	outData[gl_InvocationID].positionAndHeightMapOffset = positionAndHeightMapOffset[gl_InvocationID];
}
