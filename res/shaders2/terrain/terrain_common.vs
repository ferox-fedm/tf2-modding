...
VERTEX_INPUT(ATTR_POS_LOC, vec4, attrPosIndex, vec4(1.0));
...
vec2 getTilePos() {
	return vec2(int(uint(attrPosIndex.w) % 1024u) - 128, int(uint(attrPosIndex.w) / 1024u) - 128);
}

vec2 getTexCoord() {
	return (attrPosIndex.xy / u_terrain.terrainTileData1.x - getTilePos());
}

vec2 getTileOffset() {
	return vec2(uvec2(getTilePos() - u_terrain.terrainStart) % 4u); // terrainStart
}

void exportData() {
	texCoordtileOffset = vec4(getTexCoord(), getTileOffset());
}
