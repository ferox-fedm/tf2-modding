...
VERTEX_INPUT(ATTR_TC_LOC, float, attrTexCoord, float(1.0));

OUTPUT(POS_LOC, vec4, posAmbient);
OUTPUT(NRML_LOC, vec3, normal);
OUTPUT(TC_LOC, vec2, texCoord);
OUTPUT(TC_1_LOC, vec2, texCoord2);

void calcFogFactor(vec3 pos);

void main() {
	gl_Position = u_view.projView * attrPosition;
#ifdef RESCALE_Z_POS
	...
	normal = attrNormal;
	texCoord = vec2(attrTexCoord / 8192.0, attrPosition.z > -100.0 ? 1.0 : .0);
