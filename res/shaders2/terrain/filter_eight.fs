...
vec3 filterAndBlendMaterial(vec3 dposdx, vec3 dposdy, out vec3 normal, out vec2 ambientGloss) {
	...
	float temp2sum[maxNumIndices + 9]; // Workaround for old Intel Mac
	...
	for (int i = 0; i < min(numIndices, 8); ++i) {
