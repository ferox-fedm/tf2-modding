...
vec3 getLevelColor(vec3 pos, int iLevel, vec3 normal, 
	...
	nrml = textureGrad(detailNrmlArrayTex, vec3(pos.xy, iLevel), dposdx.xy, dposdy.xy).rg;
	vec3 col = textureGrad(overlayArrayTex, vec3(pos.xy * doSizeoStrength.y, iLevel), dposdx.xy * doSizeoStrength.y, dposdy.xy * doSizeoStrength.y).rgb;
	vec4 col0 = textureGrad(detailColorArrayTex, vec3(pos.xy, iLevel), dposdx.xy, dposdy.xy);
	col0.a = doSizeoStrength.z;
	glossAoH = textureGrad(detailGlossArrayTex, vec3(pos.xy, iLevel), dposdx.xy, dposdy.xy).gba;
	glossAoH.z += u_dataBlock.data[iLevel].w;
	
	return overlay(col0, col.rgb);
}
