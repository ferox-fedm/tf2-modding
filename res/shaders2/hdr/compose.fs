...
LAYOUT_SAMPLER(0, 0) uniform sampler2D texFramebuf;
LAYOUT_SAMPLER(0, 1) uniform sampler2D texBlurred;

LAYOUT_UBO(0, 4) uniform Blur {
	float exposure;
	float bloomFactor;
	float hdrFactor;
} u_blur;
...
#endif
    vec3 col1 = texture(texFramebuf, texCoord2.xy).rgb;
	vec3 col;
	if (u_blur.bloomFactor == 0.0) {
		col = u_blur.hdrFactor < 1.0 ? pow(col1, vec3(1.0 / 2.2)) : toneMap(col1);
	} else {
		vec3 col2 = texture(texBlurred, texCoord2.xy).rgb;
		col = u_blur.hdrFactor < 1.0 ? pow(col1, vec3(1.0 / 2.2)) : toneMap(col1 + u_blur.bloomFactor * col2);
	}

	if (u_blur.hdrFactor == 1.0) {
		float lb = .0 / 255.0;
		float ub = 210.0 / 255.0;
		col = (col - vec3(lb)) / (ub - lb);
	}
