...
#endif
	vec3 col = texture(tex, texCoord2).rgb;
	col = max(col, vec3(.0));

	float lum = desaturate(col);
	if (lum < u_lum.threshold) {
		color.rgb = vec3(.0);
		return;
	}

	float lum2 = sqrt(lum - u_lum.threshold);
	color.rgb = (lum2 / lum) * col;
}
