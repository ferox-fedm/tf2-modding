...
#endif
	vec2 size = 0.25 * u_view.extra;

	vec2 o1 = vec2(size);
	vec2 o2 = vec2(-size.x, size.y);
	vec2 o3 = vec2(size.x, -size.y);
	vec2 o4 = vec2(-size.x, -size.y);

	color = 0.25 * (texture(tex, texCoord2 + o1) + texture(tex, texCoord2 + o2) + texture(tex, texCoord2 + o3) + texture(tex, texCoord2 + o4));
}
