...
LAYOUT_SAMPLER(0, 0) uniform sampler2D tex0;
LAYOUT_SAMPLER(0, 1) uniform sampler2D tex1;
LAYOUT_SAMPLER(0, 2) uniform sampler2D tex2;
LAYOUT_SAMPLER(0, 3) uniform sampler2D tex3;
...
#endif
	vec4 cols[4] = vec4[](texture(tex0, texCoord2), texture(tex1, texCoord2),
			texture(tex2, texCoord2), texture(tex3, texCoord2));

	//float weights[4] = float[](1.0 / 15.0, 2.0 / 15.0, 4.0 / 15.0, 8.0 / 15.0);
	float weights[4] = float[](1.0 / 7.104, 1.4 / 7.104, 1.96 / 7.104, 2.744 / 7.104);

	vec4 result = vec4(.0);

	for (int i = 0; i < 4; ++i) {
		result += weights[i] * cols[i];
	}
