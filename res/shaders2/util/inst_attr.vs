...
LAYOUT_UBO(0, 12) uniform BufferState {
	int bufTexAttrsEnabled;
} u_state;

LAYOUT_SAMPLER(0, 13) uniform samplerBuffer instAttrColorBufTex;

VERTEX_INPUT(ATTR_OBJIDX_LOC, float, instAttrObjIdx, 1.0);

void getBufTexInstAttrs() {
	if (u_state.bufTexAttrsEnabled == 0) return;

}

void getBufTexInstAttrsDepth() {
	if (u_state.bufTexAttrsEnabled == 0) return;

}

void getBufTexInstAttrsSsao() {
	if (u_state.bufTexAttrsEnabled == 0) return;
...
void getBufTexInstAttrsColor(inout vec4 color) {
	...
	int index = int(instAttrObjIdx);

	color = 2.0 * texelFetch(instAttrColorBufTex, index) - vec4(1.0);
}

void getBufTexInstAttrsColor(inout vec4 color, float attrObjIdx) {
	if (u_state.bufTexAttrsEnabled == 0) return;

	int index = int(attrObjIdx);
