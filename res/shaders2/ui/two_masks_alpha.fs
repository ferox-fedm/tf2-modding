...
layout(push_constant, std140) uniform Colors {
	vec3 backgroundCol;
	vec3 foregroundCol;
	vec3 borderCol;
};

LAYOUT_SAMPLER(1, 0) uniform sampler2D mask;
LAYOUT_SAMPLER(1, 1) uniform sampler2D borderMask;
LAYOUT_SAMPLER(1, 2) uniform sampler2D alphaMask;
...
void main() {
	...
	col.rgb = mix(backgroundCol, borderCol, border);
	col.rgb = mix(col.rgb, foregroundCol, mask); 	
	col.a = alpha;	
}
