local names = require "personnameutil"

local firstNamesMale = names.netherlands.english.firstNamesMale
local firstNamesFemale = names.netherlands.english.firstNamesFemale
local lastNames = names.netherlands.english.lastNames

function data()
return {
	makeName = function (male)
		if (male) then
			return firstNamesMale[math.random(#firstNamesMale)] .. " " .. lastNames[math.random(#lastNames)]
		else
			return firstNamesFemale[math.random(#firstNamesFemale)] .. " " .. lastNames[math.random(#lastNames)]
		end
	end
}
end
